...
function upvoteComment(url, request){
  ...
  if (savedComment && username && id && database.users[username]) {
...
function loadDatabase() {
  ...
  try {
    const loadedDatabase = yaml.safeLoad(fs.readFileSync('database.yml', 'utf8'));
...
function saveDatabase() {
  const updatedDatabase = yaml.safeDump(database);
  fs.writeFile('database.yml', updatedDatabase, function (err) {
    if (err) {throw err};
  });
}
